...
const Pagination = ({ name, page, total }) => {
    ...
    return (
        <div className={classes.Pagination}>
            {parsedPage === 1 ? null : (
                <Link className={[`${classes.Pagination__Btn}`, `${classes.Pagination__Btn_Back}`].join(" ")} to={`/${section}/${name}/${parsedPage - 1}`}>
            ...
            {parsedPage >= parsedTotal ? null : (
                <Link className={[`${classes.Pagination__Btn}`, `${classes.Pagination__Btn_Forward}`].join(" ")} to={`/${section}/${name}/${parsedPage + 1}`}>
