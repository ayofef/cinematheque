...
export const Backdrop = ({id, title}) => {
    ...
    return(
        ...
        <img 
            style={{width: "100%", height: "100%", objectPosition: "50% 0%", objectFit: "cover", filter: "brightness(50%)"}}
            src={`${baseUrl}original${id}`}
            alt={title}
        />
        </LazyLoad>
    )
...
export const Img = ({id, title, style}) => {
    ...
    return(
        <LazyLoad height={300} offset={50} once={true} placeholder={<Loader />}>
            ...
            <img 
            className={style}
            src={id ? `${baseUrl}original${id}` : MovieSvg} // Upcoming movies that dont have a poster yet returns null, instead of a broken image show an svg placeholder instead
            alt={title}
            title={title}
